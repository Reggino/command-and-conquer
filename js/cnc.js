/*jslint browser: true, unparam: true, regexp: true, maxerr: 50, indent: 4 */
/*global $: false, HTMLImageElement: false, Audio: false */

$(function () {
	'use strict';

	//general
	var game, canvas, context, mouse, sidebar, ai, spriteCanvas, spriteContext, AStar,

		//objects
		buildings, turrets, infantry, vehicles, levels, fog, sounds, overlay, ships,

		//functions
		preloadImage, loadImageArray, getLife, drawSelection, underPoint, findRefineryInRange, findTiberiumInRange,
		findEnemiesInRange, findPath, loadSpriteSheet, transformSpriteSheet, findAngle, angleDiff, addAngle, shortenPath;

	preloadImage = function (imgUrl, callbackFunction) {
		var loadee = this, image;
		this.loaded = false;
		image = new Image();
		image.src = 'images/' + imgUrl;
		this.preloadCount += 1;
		$(image).bind('load', function () {
			loadee.loadedCount += 1;
	...
	loadSpriteSheet = function (forObject, details, from) {
		var i, constructImageCount, constructImageName;
		...
		for (i = 0; i < details.imagesToLoad.length; i += 1) {
			constructImageCount = details.imagesToLoad[i].count;
			constructImageName = details.imagesToLoad[i].name;
			forObject.spriteArray[constructImageName] = {
				name: constructImageName,
				count: constructImageCount,
				offset: forObject.spriteCount
			};
	...
	mouse = {
		...
		cursorLoop: 0,
		drawCursor: function () {
			var x, y, width, height, imageNumber;
			...
			if (this.dragSelect) {
				x = Math.min(this.gameX, this.dragX);
				y = Math.min(this.gameY, this.dragY);
				width = Math.abs(this.gameX - this.dragX);
				height = Math.abs(this.gameY - this.dragY);
			...
			imageNumber = this.cursor.spriteOffset + Math.floor(this.cursorLoop / this.cursor.cursorSpeed);
		...
		checkOverObject: function () {
			...
			for (i = game.overlay.length - 1; i >= 0; i -= 1) {
				overlay = game.overlay[i];
			...
			for (i = game.buildings.length - 1; i >= 0; i -= 1) {
			...
			for (i = game.turrets.length - 1; i >= 0; i -= 1) {
			...
			for (i = game.units.length - 1; i >= 0; i -= 1) {
		...
		draw: function () {
			var x, y, buildingType, grid, tooltipName, tooltipCost, hovButton, selectedObject = this.checkOverObject();
			...
			if (this.y < game.viewportTop || this.y > game.viewportTop + game.viewportHeight) {
				...
				buildingType = buildings.types[sidebar.deployBuilding] || turrets.types[sidebar.deployBuilding];
				grid = $.extend([], buildingType.gridShape);
				...
				//over a button
				hovButton = sidebar.hoveredButton();
				if (hovButton) {
					tooltipName = hovButton.type;
...
/*				if (mouse.x >= 0 && mouse.x < 160) {
		...
		listenEvents: function () {
			...
			$('#canvas').mouseup(function (ev) {
				var x1, x2, y1, y2, i, unit;
				if (ev.which === 1) {
					if (mouse.dragSelect) {
						if (!ev.shiftKey) {
							game.clearSelection();
						}
						x1 = Math.min(mouse.gameX, mouse.dragX);
						y1 = Math.min(mouse.gameY, mouse.dragY);
						x2 = Math.max(mouse.gameX, mouse.dragX);
						y2 = Math.max(mouse.gameY, mouse.dragY);
						for (i = game.units.length - 1; i >= 0; i -= 1) {
							unit = game.units[i];
	...
	game = {
		...
		drawMap: function () {
			...
			for (y = 0; y < this.currentLevel.obstructionGrid.length; y += 1) {
				game.obstructionGrid[y] = [];
				for (x = 0; x < this.currentLevel.obstructionGrid[y].length; x += 1) {
					game.obstructionGrid[y][x] = this.currentLevel.obstructionGrid[y][x];
				}
			}

			for (i = this.buildings.length - 1; i >= 0; i -= 1) {
				bldng = this.buildings[i];
				for (y = 0; y < bldng.gridShape.length; y += 1) {
					for (x = 0; x < bldng.gridShape[y].length; x += 1) {
			...
			for (i = this.turrets.length - 1; i >= 0; i -= 1) {
				game.obstructionGrid[this.turrets[i].y][this.turrets[i].x] = 1;
			}

			for (i = this.units.length - 1; i >= 0; i -= 1) {
				unit = this.units[i];
				if (!unit.moving) {
			...
			for (i = this.overlay.length - 1; i >= 0; i -= 1) {
				over = this.overlay[i];
				if (over.name === 'tree') {
		...
		drawGrid: function () {
			var gridSize, mapWidth, mapHeight, viewportX, viewportY, gridWidth, gridHeight, i, j;
			gridSize = game.gridSize;
			mapWidth = game.currentLevel.mapImage.width;
			mapHeight = game.currentLevel.mapImage.height;
			...
			for (i = 0; i < gridWidth; i += 1) {
				context.moveTo((i * gridSize - viewportX + game.viewportLeft), -viewportY + game.viewportTop);
				context.lineTo(i * gridSize - viewportX + game.viewportLeft, mapHeight - viewportY + game.viewportTop);
			}
			for (i = 0; i < gridHeight; i += 1) {
				context.moveTo(-viewportX + game.viewportLeft, i * gridSize - viewportY + game.viewportTop);
				context.lineTo(mapWidth - viewportX + game.viewportLeft, i * gridSize - viewportY + game.viewportTop);
			}
			context.stroke();

			for (i = game.obstructionGrid.length - 1; i >= 0; i -= 1) {
				for (j = game.obstructionGrid[i].length - 1; j >= 0; j -= 1) {
		...
		drawBullets: function () {
			var bullet, i, j, x, y, overObject;
			for (j = this.bullets.length - 1; j >= 0; j -= 1) {
				...
				x = (bullet.x * game.gridSize);
				y = (bullet.y * game.gridSize);
				//alert(x + ' ' + y)
				...
				if (!bullet.dead) {
					...
					if (overObject) {
						...
						overObject.health = overObject.health - Math.floor((bullet.damage || 10) + 10 * Math.random());
		...
		drawObjects: function () {
			...
			for (i = this.selectedAttackers.length - 1; i >= 0; i -= 1) {
				if (this.selectedAttackers[i].status === 'destroy') {
					this.selectedAttackers.splice(i, 1);
				}
			}

			for (i = this.selectedUnits.length - 1; i >= 0; i -= 1) {
		...
		showDebugger: function () {
			var html, getKeys = function (item) {
				var html = '<ul>', key, o;
				for (key in item) {
		...
		missionStatus: function () {
			var heroUnits = [], heroBuildings = [], heroTurrets = [], villainBuildings = [], villainUnits = [],
				villainTurrets = [], i, item;
		...
		click: function (ev, rightClick) {
			var selectedObject, buildingType, grid, x, y, i;
			...
			selectedObject = mouse.checkOverObject();
			if (rightClick) {
				this.clearSelection();
			...
			if (sidebar.repairMode) {
				...
				if (selectedObject) {
					if (game.selectedUnits.length === 1 && selectedObject.selected && selectedObject.team === game.currentLevel.team) {
						...
						for (i = game.selectedAttackers.length - 1; i >= 0; i -= 1) {
						...
						if (game.selectedUnits.length > 0) {
							if (!(game.obstructionGrid[mouse.gridY] && game.obstructionGrid[mouse.gridY][mouse.gridX] === 1)) {
								for (i = game.selectedUnits.length - 1; i >= 0; i -= 1) {
		...
		start: function () {
			...
			this.tiberiumLoop = setInterval(function () {
				var overlay, i;
				for (i = 0; i < game.overlay.length; i += 1) {
					overlay = game.overlay[i];
	...
	sidebar = {
		...
		finishDeployingBuilding: function () {
			var i;
			for (i = 0; i < game.buildings.length; i += 1) {
		...
		finishDeployingUnit: function (unitButton) {
			...
			if (unitButton.type === 'infantry') {
				...
				vehicle = vehicles.add({name: unitButton.name, x: constructedAt.x + 1,
			...
			//sounds.play('construction')
			//sidebar.deployMode = false;
			for (i = this.rightButtons.length - 1; i >= 0; i -= 1) {
		...
		hoveredButton: function () {
			var buttonPressed, buttonPressedIndex, buttons, clickX, clickY, buttonPosition, i;
			clickY = mouse.y - sidebar.top;
			clickX = mouse.x;
			if (clickY >= 165 && clickY <= 455) {
				...
				if (buttons && buttons.length > buttonPressedIndex) {
					buttonPressed = buttons[buttonPressedIndex];
		...
		click: function (ev, rightClick) {
			var clickX, clickY, i, buttonPosition, buttonPressedIndex, buttons, buttonPressed;
			clickY = mouse.y - this.top;
			clickX = mouse.x;
			//alert(2)
			// press a top button
		...
		checkDependency: function () {
			var i, j, k, button, dependenciesSatisfied, found, dependency, building, buttonFound, foundIndex;
			//alert(this.allButtons.length);
			for (i = 0; i < this.allButtons.length; i += 1) {
				button = this.allButtons[i];
				dependenciesSatisfied = true;
				//alert(button.dependency.length);
				for (j = button.dependency.length - 1; j >= 0; j -= 1) {
					found = false;
					dependency = button.dependency[j];
					for (k = game.buildings.length - 1; k >= 0; k -= 1) {
						building = game.buildings[k];
				...
				if (button.type === 'building') {
					...
					for (j = this.leftButtons.length - 1; j >= 0; j -= 1) {
					...
					for (j = this.rightButtons.length - 1; j >= 0; j -= 1) {
		...
		drawButton: function (side, index) { //side is left or right; index is 0 to 5
			var buttons, offset, button, xOffset, yOffset;
			buttons = (side === 'left') ? this.leftButtons : this.rightButtons;
			offset = (side === 'left') ? this.leftButtonOffset : this.rightButtonOffset;
			button = buttons[index + offset];
			xOffset = (side === 'left') ? 500 : 570;
			yOffset = 165 + this.top + index * this.iconHeight;
		...
		processButton: function (side, index) { //side is left or right; index is 0 to 5
			var buttons, offset, button; //, xOffset, yOffset;
			buttons = (side === 'left') ? this.leftButtons : this.rightButtons;
			offset = 0;// (side=='left')?this.leftButtonOffset: this.rightButtonOffset;
			button = buttons[index + offset];
			//xOffset = (side === 'left') ? 500 : 570;
			//yOffset = 165 + this.top + index * this.iconHeight;
		...
		powerScale: 4,
		checkPower: function () {
			var offsetX, offsetY, barHeight, barWidth, building, k, red, orange, green;
			offsetX = this.left;
			offsetY = this.top + 160;
			barHeight = 320;
			barWidth = 20;
			...
			red = 'rgba(174,52,28,0.7)';
			//var red = 'rgba(240,75,35,0.6)';
			orange = 'rgba(250,100,0,0.6)';
			//var green = 'rgba(48,85,44,0.6)';
			green = 'rgba(84,252,84,0.3)';
		...
		draw: function () {
			var c, i, maxLeft, maxRight;
			context.drawImage(this.tabsImage, 0, this.top - this.tabsImage.height + 2);
			context.fillStyle = 'lightgreen';
			context.font = '12px "Command and Conquer"';
			// convert the cash score to a string and space separate to print it properly
			c = String(this.cash).split('').join(' ');
			...
			for (i = 0; i < this.leftButtons.length; i += 1) {
				this.processButton('left', i);
			}
			for (i = 0; i < this.rightButtons.length; i += 1) {
			...
			if (this.visible) {
				...
				this.checkPower();
				maxLeft = this.leftButtons.length > 6 ? 6 : this.leftButtons.length;
				for (i = 0; i < maxLeft; i += 1) {
					this.drawButton('left', i);
				}
				maxRight = this.rightButtons.length > 6 ? 6 : this.rightButtons.length;
				for (i = 0; i < maxRight; i += 1) {
	...
	buildings = {
		...
		loadedCount: 0,
		draw: function () {
			var teamYOffset = 0, imageList, imageWidth, imageHeight, baseImage, imageCategory, imageIndex, subtractAmount,
					cashSpent, 	life = this.getLife();;
			...
			imageWidth = this.gridShape[0].length * game.gridSize;
			imageHeight = this.spriteImage.height;

			// Then draw the base with baseOffset
			baseImage = this.spriteArray[this.life + "-base"];
			...
			imageList = this.spriteArray[imageCategory];
			if (!this.animationIndex) {
				this.animationIndex = 0;
			}
			if (imageList.count >= Math.floor(this.animationIndex / this.animationSpeed)) {
				imageIndex = Math.floor(this.animationIndex / this.animationSpeed);
				if (this.status === 'sell') {
				...
				if (this.name === 'refinery' && (this.status === "build" || this.status === 'unload')) {
					if (this.status === 'build') {
						...
						if (this.harvester.tiberium) {
							subtractAmount = this.harvester.tiberium > 4 ? 5 : this.harvester.tiberium;
							if (this.team === game.currentLevel.team) {
		...
		add: function (details) {
			var newBuilding = {}, name = details.name;
	...
	transformSpriteSheet = function (forObject, details) {
		var spriteContext, colorMap, imgData, imgDataArray, size, p, r, g, b, a, i;
		...
		spriteContext = forObject.spriteCanvas.getContext('2d');
		...
		imgData = spriteContext.getImageData(0, 0, forObject.spriteCanvas.width, forObject.spriteCanvas.height);
		imgDataArray = imgData.data;
		size = imgDataArray.length / 4;
		//console.log(size + " " + details.type + ' ' + details.name);
		for (p = size / 2; p < size; p += 1) {

			//console.log(p);
			r = imgDataArray[p * 4];
			g = imgDataArray[p * 4 + 1];
			b = imgDataArray[p * 4 + 2];
			a = imgDataArray[p * 4 + 2];
			...
			if (details.type === 'turret' || details.type === 'building' || details.name === 'mcv' || details.name === 'harvester') {
				...
				for (i = colorMap.length - 1; i >= 0; i -= 1) {
					//alert(1);
					if (r === colorMap[i].gdi[0] && g === colorMap[i].gdi[1] && b === colorMap[i].gdi[2]) {
						imgDataArray[p * 4] = colorMap[i].nod[0];
	...
	infantry = {
		...
		collision: function (otherUnit) {
			var distanceSquared, radiusSquared;
			if (this === otherUnit) {
				return false;
			}
			distanceSquared = Math.pow(this.x - otherUnit.x, 2) + Math.pow(this.y - otherUnit.y, 2);
			radiusSquared = Math.pow((this.collisionRadius + otherUnit.collisionRadius) / game.gridSize, 2);
			//alert(distanceSquared +' '+ radiusSquared);
			return distanceSquared <= radiusSquared;
		},
		load: function (name) {
			var details, infantryType, i, constructImageCount, constructImageDirectionCount, constructImageName, imgArray, j;
			details = this.infantryDetails[name];
			infantryType = {};
			...
			for (i = details.imagesToLoad.length - 1; i >= 0; i -= 1) {
				constructImageCount = details.imagesToLoad[i].count;
				constructImageDirectionCount = details.imagesToLoad[i].directionCount;
				constructImageName = details.imagesToLoad[i].name;
				imgArray = [];
				for (j = 0; j < constructImageDirectionCount; j += 1) {
		...
		draw: function () {
			var imageList, moveImage, x, y;

			//alert(this.status);
			//alert(this.imageArray[this.status][this.moveDirection]);
			imageList = this.imageArray[this.status][this.moveDirection];
			...
			moveImage = imageList[Math.floor(this.animationIndex / this.animationSpeed)];
			//alert(this.moveOffsetX);
			x = this.x * game.gridSize + game.viewportAdjustX + this.pixelOffsetX;
			y = this.y * game.gridSize + game.viewportAdjustY + this.pixelOffsetY;
			//context.drawImage

			//@todo find out whether the function below is needed?
		...
		move: function () {
			...
			this.speedCounter += 1;
			angle = (this.moveDirection / 8) * 2 * Math.PI; //Math.round( (90+(unit.direction/32)*360)%360);
			///alert(angle);
			if (this.status === 'walk') {
			...
			if (this.speedCounter >= 7) {
				...
				this.moveDirection = Math.floor(this.moveDirection + (Math.round((Math.random() - 0.5) * 10) / 10));
	...
	vehicles = {
		...
		loadedCount: 0,
		collision: function (otherUnit) {
			var distanceSquared, radiusSquared, softHardRadiusSquared, softRadiusSquared;
			if (this === otherUnit) {
				return false;
			}
			...
			distanceSquared = Math.pow(this.x - otherUnit.x, 2) + Math.pow(this.y - otherUnit.y, 2);
			radiusSquared = Math.pow((this.collisionRadius + otherUnit.collisionRadius) / game.gridSize, 2);
			softHardRadiusSquared = Math.pow((this.softCollisionRadius + otherUnit.collisionRadius) / game.gridSize, 2);
			softRadiusSquared = Math.pow((this.softCollisionRadius + otherUnit.softCollisionRadius) / game.gridSize, 2);
		...
		loadSpriteSheet: loadSpriteSheet,
		draw: function () {
			var imageWidth, imageHeight, x, y, teamYOffset, imageList, imageIndex, turretList;
			// Finally draw the top part with appropriate animation
			imageWidth = this.pixelWidth;
			imageHeight = this.pixelHeight;
			x = Math.round(this.x * game.gridSize + this.pixelOffsetX + game.viewportAdjustX);
			y = Math.round(this.y * game.gridSize + this.pixelOffsetY + game.viewportAdjustY);
			teamYOffset = 0;
			if (this.team !== game.currentLevel.team) {
				teamYOffset = this.pixelHeight;
			}
			...
			if (this.status === "") {
				imageList = this.spriteArray.move;
				imageIndex = Math.floor(this.moveDirection);
				...
				imageList = this.spriteArray[this.status];
				if (imageList.count >= Math.floor(this.animationIndex / this.animationSpeed)) {
					imageIndex = Math.floor(this.animationIndex / this.animationSpeed);
					context.drawImage(this.spriteCanvas, (imageList.offset + imageIndex) * imageWidth, teamYOffset, imageWidth, imageHeight, x, y, imageWidth, imageHeight);
				}
			...
			if (this.turretDirection >= 0) {
				turretList = this.spriteArray.turret;
				if (turretList) {
					imageIndex = Math.floor(this.turretDirection);
			...
			if (game.debugMode) {
				...
				if (this.orders.to) {
					context.fillText(this.orders.to.x + ',' + this.orders.to.y, x, y + 20);
				}
			}
			if (game.debugMode) {
		...
		moveTo: function (destination, turretAtTarget) {
			var start, end, newAngle, movement, angleRadians, collision, k, l, tile, collDirection, dTurn, dTurnDestination,
				magTurn, collision2, turretAngle;
			start = [Math.floor(this.x), Math.floor(this.y)];
			...
			if (this.path.length > 1) {
				newAngle = findAngle(this.path[1], this.path[0], 32);
				movement = this.movementSpeed * game.speedAdjustmentFactor / game.gridSize;
				angleRadians = (this.moveDirection / 32) * 2 * Math.PI;
				this.x = (this.x - movement * Math.sin(angleRadians));
				this.y = (this.y - movement * Math.cos(angleRadians));
				this.colliding = false;
				for (k = game.units.length - 1; k >= 0; k -= 1) {
					collision = this.collision(game.units[k]);
					if (collision) {
				...
				if (this.colliding) {
					//his.movementSpeed = 0;
					collDirection = findAngle(this.collisionWith, this, 32);
					dTurn = angleDiff(this.moveDirection, collDirection, 32);
					dTurnDestination = angleDiff(newAngle, collDirection, 32);
					...
					case 'soft':
						if (Math.abs(dTurn) === 0) { // Bumping into something ahead
							...
							newAngle = addAngle(this.moveDirection, -dTurn, 32);
							//console.log('moving: ' + this.moveDirection +' coll: '+this.collisionType+' '+collDirection + ' dTurn: ' +dTurn + ' RESULT: newAngle: ' +newAngle +' speed: '+this.movementSpeed);
							//this.moveDirection = newAngle;
						} else if (Math.abs(dTurn) < 4) {
							this.movementSpeed = this.speed * (this.collisionDistance - this.collisionRadius) / (this.softCollisionRadius - this.collisionRadius);
				...
				magTurn = Math.abs(angleDiff(this.moveDirection, newAngle, 32));
				...
				for (k = 0; k < game.obstructionGrid.length; k += 1) {
					for (l = 0; l < game.obstructionGrid[k].length; l += 1) {
						if (game.obstructionGrid[k][l] > 0) {
							...
							tile = {x: (l + 0.5), y: (k + 0.5),
								collisionRadius: game.gridSize * 0.5, softCollisionRadius: game.gridSize * 0.7};
							collision2 = this.collision(tile);
							if (collision2) {
								break;
							}
						}
		...
		processOrders: function () {
			var distance, destination, targetX, targetY, targetCGX, targetCGY, enemiesInRange, enemy, reachedThreshold, turretAngle;
			this.colliding = false;
			this.collisionType = '';
			...
			if (this.orders.type === 'harvest') {
				...
				distance = Math.pow(Math.pow(this.orders.to.y + 0.5 - this.y, 2) + Math.pow(this.orders.to.x + 0.5 - this.x, 2), 0.5);
				...
				destination = {x: this.orders.to.x, y: this.orders.to.y + 2};
				distance = Math.pow(Math.pow(destination.y - this.y, 2) + Math.pow(destination.x - this.x, 2), 0.5);
				//alert(distance);
				...
				reachedThreshold = this.softCollisionRadius / game.gridSize < 0.5 ? 0.5 + this.softCollisionRadius / game.gridSize : this.softCollisionRadius / game.gridSize;
				...
				enemiesInRange = findEnemiesInRange(this, 2);
				...
				this.moveTo(this.orders.to);
				distance = Math.pow(Math.pow(this.orders.to.y - this.y, 2) + Math.pow(this.orders.to.x - this.x, 2), 0.5);
				...
				targetX = this.orders.target.x;
				targetY = this.orders.target.y;
				targetCGX = this.orders.target.x;
				targetCGY = this.orders.target.y;
				...
				if (Math.pow(targetX - this.x, 2) + Math.pow(targetY - this.y, 2) <= Math.pow(this.sight, 2)) {
					if (this.orders.type === 'attack') {
						turretAngle = findAngle({x: targetCGX, y: targetCGY}, this, 32);
						if (this.turretDirection === turretAngle) {
		...
		move: function () {
			var movement, angle, instr, i, delta;
			...
			for (i = 0; i < this.instructions.length; i += 1) {
				...
				if (instr.type === 'move') {
					...
					angle = (this.moveDirection / 32) * 2 * Math.PI;
				...
				if (instr.type === 'aim') {
					...
					if (instr.toDirection === this.turretDirection) {
						...
						delta = angleDiff(Math.floor(this.turretDirection), Math.floor(instr.toDirection), 32);
						if (Math.abs(delta) < 1) {
				...
				if (instr.type === 'fire') {
					...
					if (!this.bulletFiring) {
						...
						angle = (this.turretDirection / 32) * 2 * Math.PI;
	...
	turrets = {
		...
		loadedCount: 0,
		load: function (name) {
			var details, turretType;
			details = this.turretDetails[name];
			turretType = {};
			turretType.defaults = {
				type: 'turret',
				status: '',
		...
		loadSpriteSheet: loadSpriteSheet,
		draw: function () {
			var life, teamYOffset, imageList, imageWidth, imageHeight, x, y, imageIndex, imageCategory, turretList;
			life = this.getLife();
			teamYOffset = 0;
			if (this.team !== game.currentLevel.team) {
				teamYOffset = this.pixelHeight;
			...
			x = this.x * game.gridSize + game.viewportAdjustX;
			y = this.y * game.gridSize + game.viewportAdjustY;
			if (this.status === "") {
				imageIndex = Math.floor(this.turretDirection);
				context.drawImage(this.spriteCanvas, (imageList.offset + imageIndex) * imageWidth, teamYOffset, imageWidth, imageHeight, x, y, imageWidth, imageHeight);
			} else {
		...
		processOrders: function () {
			var targetX, targetY, turretAngle, enemiesInRange, enemy;
			...
			if (this.orders.type === 'attack') {
				...
				// first see if an evil unit is in sight and track it :);
				enemiesInRange = findEnemiesInRange(this, 0);
				if (enemiesInRange.length > 0) {
					enemy = enemiesInRange[0];
	...
	overlay = {
		...
		loadSpriteSheet: loadSpriteSheet,
		load: function (name) {
			var details, overlayType = {
				name: name,
				draw: this.draw
			};
			details = this.overlayDetails[name];

			this.loadSpriteSheet(overlayType, details, 'tiles/temperate');
			/*
			 var imageArray = [];
			 for(i=0;i<details.count;i += 1){
		...
		draw: function () {
			var imageWidth, imageHeight, x, y, imageList, imageIndex;

			// Finally draw the top part with appropriate animation
			imageWidth = this.pixelWidth;
			imageHeight = this.pixelHeight;

			x = Math.round((this.x + this.gridOffsetX) * game.gridSize + game.viewportAdjustX);
			y = Math.round((this.y + this.gridOffsetY) * game.gridSize + game.viewportAdjustY);

			imageList = this.spriteArray[this.type];
			imageIndex = this.stage;
	...
	levels = {
					...
					actions: [
						{
							...
							trigger: 'condition',
							condition: function () {
								// @todo define proper condition: units.enemyUnitCount() does not exist
								return false;
								//(units.enemyUnitCount() === 0 && buildings.enemyBuildingsCount === 0);
							}
						},
						{
							action: 'endLevel',
		...
		loadedCount: 0,
		load: function (id) {
			var level = {}, obstructionGrid, mapGrid, y, x, i, terrain, overlayArray, details, self = this;
			level.id = id;
			//level.mapImage = new Image();
			level.mapImage = this.preloadImage(this.levelDetails[id].mapUrl);
			details = this.levelDetails[id];
			$.each(details.items, function (item) {
				var i;
				if (item === "vehicles") {
					for (i = details.items[item].length - 1; i >= 0; i -= 1) {
						vehicles.load(self.levelDetails[id].items[item][i]);
					}
			...
			obstructionGrid = [];
			mapGrid = [];
			for (y = 0; y < details.gridHeight; y += 1) {
				obstructionGrid[y] = [];
				mapGrid[y] = [];
				for (x = 0; x < details.gridWidth; x += 1) {
					obstructionGrid[y][x] = 0;
				}
			}

			for (i = details.terrain.length - 1; i >= 0; i -= 1) {
				terrain = details.terrain[i];
				for (x = terrain.x1; x <= terrain.x2; x += 1) {
					for (y = terrain.y1; y <= terrain.y2; y += 1) {
			...
			overlayArray = [];
			for (i = details.overlay.length - 1; i >= 0; i -= 1) {
	...
	sounds = {
		...
		play: function (name, unique) {
			var options = this.sound_list[name], i;
			//alert(name);
			if (options.length === 1) {
				options[0].play();
			} else {
	...
	loadImageArray = function (imgName, count, extn) {
		var imageArray = [], i;
		if (!extn) {
			extn = '.png';
		}
	...
	drawSelection = function () {
		var selectBarSize, x, y, x1, x2, y1, y2;
		if (this.selected) {
			context.strokeStyle = 'white';
	...
	underPoint = function (x, y) {
		var xo, yo, x1, y1, x2, y2;
		xo = this.x * game.gridSize + this.pixelOffsetX;
		yo = this.y * game.gridSize + this.pixelOffsetY;

		x1 = xo + this.pixelLeft;
		y1 = yo + this.pixelTop;
		x2 = x1 + this.pixelWidth;
		y2 = y1 + this.pixelHeight;
		//
	...
	findRefineryInRange = function (hero) {
		var currentDistance, currentRefinery, i, building, distance;
		if (!hero) {
			hero = this;
		}
		for (i = 0; i < game.buildings.length; i += 1) {
			...
			if (building.name === 'refinery' && building.team === hero.team) {
				distance = Math.pow(building.x - hero.x, 2) + Math.pow(building.y - hero.y, 2);
	...
	findTiberiumInRange = function (hero) {
		var currentDistance, currentOverlay, overlay, i, distance;
		if (!hero) {
			hero = this;
		}
		for (i = 0; i < game.overlay.length; i += 1) {
			...
			if (overlay.name === 'tiberium' && overlay.stage > 0) {
				distance = Math.pow(overlay.x - hero.x, 2) + Math.pow(overlay.y - hero.y, 2);
				if (!currentDistance || (currentDistance > distance)) {
	...
	findEnemiesInRange = function (hero, increment) {
		var i, enemies = [], test;
	...
	findPath = function (start, end) {
		var g = game.obstructionGrid, path, k;
		...
		path = new AStar(g, start, end, 'Euclidean');
		shortenPath(path, g);
	...
	fog = {
		...
		draw: function () {
			var i, fogCanvas, fogContext, currentMap, unit, build, turret;
			fogCanvas = this.fogCanvas;
			fogContext = this.fogContext;
			currentMap = game.currentLevel.mapImage;
			fogContext.save();
			...
			context.drawImage(
				this.fogCanvas,
				game.viewportX * this.canvasWidth / currentMap.width,
				game.viewportY * this.canvasHeight / currentMap.height,
				game.viewportWidth * this.canvasWidth / currentMap.width,
				game.viewportHeight * this.canvasHeight / currentMap.height,
				game.viewportLeft,
				game.viewportTop,
				game.viewportWidth,
				game.viewportHeight
	...
	angleDiff = function (angle1, angle2, base) {
		var diff;
	...
	addAngle = function (angle, increment, base) {
	...
	findAngle = function (object, unit, base) {
		var dx, dy, angle;
		if (!base) {
			base = 32;
		}
		...
		angle = base / 2 + Math.round(Math.atan2(dx, dy) * base / (2 * Math.PI));
	...
	shortenPath = function (path, grid) {
		var nextCellVisible, start, next, slope, deltaY, y, x, deltaX, test;

		//alert(1);
		//return;
		nextCellVisible = true;
		start = path[0];
		//alert(0);
		while (nextCellVisible && path.length > 2) {
			...
			if (Math.abs(next.y - start.y) > Math.abs(next.x - start.x)) {
				...
				slope = (next.x - start.x) / (next.y - start.y);
				deltaY = 0.4 * (next.y - start.y) / Math.abs((next.y - start.y));
				y = deltaY;
				...
				//alert(2);
				slope = (next.y - start.y) / (next.x - start.x);
				deltaX = 0.4 * (next.x - start.x) / Math.abs(next.x - start.x);
				x = deltaX;
	...
	//Stopped Jslint checking from here... Don't want to mess with external / vendor code.
	var AStar = (function () {
		/**
